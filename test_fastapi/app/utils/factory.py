...
class Response(object):
    ...
    def dict_model_to_dict(obj):
        for k, v in obj.items():
            if isinstance(v, dict):
                Response.dict_model_to_dict(v)
            elif isinstance(v, list):
                obj[k] = Response.model_to_list(v)
            else:
                obj[k] = Response.model_to_dict(v)
        return obj
    ...
    def parse_sql_result(data: list):
        ...
        return columns, [Response.json_serialize(obj) for obj in data]
    ...
    def success(data=None, code=0, msg="操作成功", exclude=()):
        return Response.encode_json(dict(code=code, msg=msg, data=data), *exclude)

    @staticmethod
    def records(data: list, code=0, msg="操作成功"):
        return dict(code=code, msg=msg, data=Response.model_to_list(data))

    @staticmethod
    def success_with_size(data=None, code=0, msg="操作成功", total=0):
        if data is None:
            return Response.encode_json(dict(code=code, msg=msg, data=list(), total=0))
        return Response.encode_json(dict(code=code, msg=msg, data=data, total=total))
